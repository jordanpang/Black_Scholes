...
class BlackScholes:
    ...
    def theta(self):
        ...
        call_theta = -S * norm.pdf(d1, 0, 1) * sigma / (2 * np.sqrt(t)) - r * X * np.exp(-r * t) * norm.cdf(d2, 0, 1)
        put_theta = -S * norm.pdf(d1, 0, 1) * sigma / (2 * np.sqrt(t)) + r * X * np.exp(-r * t) * norm.cdf(-d2, 0, 1)
...
def main():
    ...
    # Create two columns for contact and picture
    ...
    with col2:
        st.image("/Users/jordan/Documents/Documents - Jordan’s MacBook Air/Black_Scholes/image2.jpeg", width=300)
    ...
    with tab2:
        ...
        st.write("Please adjust the follow inputs to compute your theoretical value of options.")
        ...
        st.markdown("Details on the Greek risk measures associated to each option are given below: ")
    ...
    with tab3:
        ...
        """)
        spot_min = st.number_input('Min Spot Price', min_value=0.01, value=S*0.9, step=0.01)
        spot_max = st.number_input('Max Spot Price', min_value=0.01, value=S*1.1, step=0.01)
        vol_min = st.number_input('Min Volatility', min_value=0.01, max_value=1.0, value=sigma*0.5, step=0.01)
        vol_max = st.number_input('Max Volatility', min_value=0.01, max_value=1.0, value=sigma*1.5, step=0.01)
        ...
        def plot_heatmap(spot_min, spot_max, vol_min, vol_max, r, X, t):
            ...
            # Heatmap number 1
            sns.heatmap(call_prices, ax=ax[0], cmap=custom_cmap, cbar_kws={'label': 'Call Price'})
            ...
            ax[0].set_xticklabels([f'{spot_min + (spot_max - spot_min) * i / (num_points - 1):.2f}' for i in np.linspace(0, num_points - 1, 10, dtype=int)], rotation=0)
            ax[0].set_yticks(np.linspace(0, num_points - 1, 10, dtype=int))
            ax[0].set_yticklabels([f'{vol_min + (vol_max - vol_min) * i / (num_points - 1):.2f}' for i in np.linspace(0, num_points - 1, 10, dtype=int)], rotation=0)
            ax[0].invert_yaxis()  # Invert y-axis

            # Heatmap number 2
            sns.heatmap(put_prices, ax=ax[1], cmap=custom_cmap, cbar_kws={'label': 'Put Price'})
            ...
            ax[1].set_xticklabels([f'{spot_min + (spot_max - spot_min) * i / (num_points - 1):.2f}' for i in np.linspace(0, num_points - 1, 10, dtype=int)], rotation=0)
            ax[1].set_yticks(np.linspace(0, num_points - 1, 10, dtype=int))
            ax[1].set_yticklabels([f'{vol_min + (vol_max - vol_min) * i / (num_points - 1):.2f}' for i in np.linspace(0, num_points - 1, 10, dtype=int)], rotation=0)
        ...
        st.write("---")
        st.subheader("P&L")
        st.write("Now we can explore the P&L of a call option of your choice for different volatilities and underlying price, fixing the rest of the inputs.")

        # Add user inputs for P&L heatmap
        X_pnl = st.number_input("Strike Price for P&L", min_value=0.01, value=100.0, step=0.01)
        t_pnl = st.number_input("Time to Expiration (Years) for P&L", min_value=0.01, value=1.0, step=0.01)
        r_pnl = st.number_input("Interest Rate for P&L", min_value=0.01, value=0.05, step=0.01)
        premium = st.number_input("Option Premium (Cost)", min_value=0.01, value=10.0, step=0.01)  # User input for the option premium
        ...
        def plot_pnl_heatmap_with_bins(spot_min, spot_max, vol_min, vol_max, r, X, t, premium, bins=10):
            ...
            sns.heatmap(pnl_values, cmap=custom_cmap1, cbar_kws={'label': 'P&L'}, ax=ax,
                        ...
                        annot=True, fmt=".2f", annot_kws={"size": 8})  # Adjust annot_kws size if needed
