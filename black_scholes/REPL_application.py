...
This module provides a REPL (Read-Eval-Print Loop) application using the
Black-Scholes Theoretical Pricing Model. It is designed as a starting point
to showcase the calculation of European
options pricing.
...
def black_scholes(r, X, S, t, sigma, option_type):
    ...
    try:
        ...
        if X < 0 or S < 0:
            raise ValueError(
                "Strike price (X) and underlying price (S) must be \
                    non-negative."
            )
        ...
        if option_type == "c":
            ...
            raise ValueError(
                "Invalid option type. Please use 'c' for call or 'p' for put."
            )
...
def main():
    """Run the black_scholes function."""
    while True:
        user = input("Type 'y' to continue, 'n' to exit: ").strip().lower()
        if user == "y":
            try:
                ...
                sigma = float(input("Enter annual volatility (e.g., 0.2 \
                                    for 20%): "))
                option_type = (
                    input("Enter 'c' for a call or 'p' for a put : ")
                    .strip()
                    .lower()
                )
                ...
                if price is not None:
                    print(f"The theoretical price of the option is: \
                          {round(price, 3)}")

            except ValueError:
                print(
                    "Invalid input. Please ensure all numerical inputs are \
                    valid numbers."
                )
